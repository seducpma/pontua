  ...
  def index
    if params[:professor].present?
      if params[:type].to_i == 1
        @titulo_busca = TituloProfessor.paginate(:all,:page=>params[:page],:per_page =>20,:conditions =>['professor_id = ? and titulo_id in (6,7,8) and ano_letivo = ?',params[:professor],Time.current.strftime("%Y")])
      else
        @titulo_busca = TituloProfessor.paginate(:all,:page=>params[:page],:per_page =>20,:conditions =>['professor_id = ? and (titulo_id in (1,2,3,4,5))',params[:professor]])
      end
    else
  ...
  def guarda_valor
    ...
    if $id_titulo.to_i == 8 or $id_titulo.to_i == 6
      ...
      if $id_titulo.to_i == 1 || $id_titulo.to_i == 2 || $id_titulo.to_i == 3 || $id_titulo.to_i == 4 || $id_titulo.to_i == 5
        ...
        if $id_titulo.to_i == 7
          render :update do |page|
            page.replace_html 'valor', :text => 'Pontualçao do Título: ' + ($valor).to_s
            page.replace_html 'qtde', :text => "<input id='titulo_professor_quantidade' type='text' value='0' size='10' name='titulo_professor[quantidade]'>"
            page.replace_html 'tipo_titulo', :text => "CURSO Á DISTÂNCIA SOMENTE CARGA HORÀRIA SUPERIOR À 30 HORAS.  desmarcar =>  <input id='titulo_professor_tipo_curso' type='checkbox' value='1' name='titulo_professor[tipo_curso]' checked='checked'> Presencial"
          end
        end
      end
    end
